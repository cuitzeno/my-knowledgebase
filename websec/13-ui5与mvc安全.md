...
- 绑定表达式或模板若拼接不可信数据，可能触发 XSS（尤其 DOM XSS，见第 9 篇）。
- 前端"隐藏"字段并不安全，敏感逻辑/鉴权必须放服务端。

## ③ 核心概念拆解

- **输出编码是默认防线**：UI5 的绑定渲染通常对文本做编码；但使用 `innerHTML` 类 API、或 `formattedText` 等会渲染 HTML 的控件时，需明确净化（sanitize）。
...
- **误区 1**：框架自动安全。默认编码能挡大部分，但 `innerHTML`/HTML 控件仍会绕过。
- **误区 2**：前端隐藏字段=权限控制。必须用服务端鉴权。
- **误区 3**：只用 `textContent` 等价物就够。第三方富文本/HTML 渲染仍需净化。
