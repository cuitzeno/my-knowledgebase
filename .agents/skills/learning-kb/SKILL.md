...
### 5. 逐篇生成
对大纲每篇，**先按文档类型选模板**（见“文档类型识别与模板映射”），再按 `templates/article-template.md` 对应段落生成 `./<theme>/NN-<中文标题>.md`：
...
- 末行"下一篇：…"先留占位，全部生成后回填（`templates/article-template.md` 通用尾部规则）。"下一篇"用相对链接，如 `(02-xxx.md)`。
...
读取素材后，**先判断文档类型**，再按对应模板生成大纲与正文。模板库见 `templates/ai-summary-templates.md`。

### 判断流程
1. **提取前 2000 字**（含目录/标题/首段），扫描关键词。
2. **按优先级匹配**：

| 优先级 | 关键词特征 | 类型 | 模板 |
|--------|------------|------|------|
| 1 | API、SDK、CLI、框架、库、工具、配置、部署、代码示例、安装、快速开始 | 技术实操类 | 模板 1：核心卡片 |
| 2 | 考试、认证、证书、大纲、知识点、章节、习题、真题、考点、重点 | 考证培训类 | 模板 2：备考复习 |
| 3 | 理论、模型、原理、方法论、范式、架构模式、设计原则、公式、推导、证明 | 理论学术类 | 模板 3：费曼学习法 |
| 4 | 博客、文章、报告、白皮书、新闻、观点、论述、分析、趋势、案例研究 | 长文泛读类 | 模板 4：金字塔原理 |

3. **混合类型处理**：若同时命中多类（如“某框架的设计原理+实战”），主类型按**用户意图**或**内容占比**决定；大纲阶段可拆分为多篇，每篇套用最贴切模板。

### 生成差异化大纲
- **模板 1/2/3**：每篇聚焦 1 个核心知识点，目标 1200–1500 字，含代码/表格/对比/案例。
- **模板 4**：先出“金字塔拆解”总览（1 篇，~800 字），再判断是否需深入拆解为多篇详细文章。

### 交互确认时同步告知
打印大纲时，额外输出：
```
检测到文档类型：<类型名> → 将套用 <模板名>
预计 N 篇，第 1 篇用模板 X，第 2 篇用模板 Y...
```
