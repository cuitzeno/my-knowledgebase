...
**WSTG（Web Security Testing Guide）** 是 OWASP 维护的开放标准，定义了 Web/API 应用安全测试的**方法论 + 检查清单**：每个测试项都写明"目标、怎么测、预期、结果判读"。最新版为 **5.0 开发线**（仓库 https://github.com/OWASP/wstg；上一稳定版为 v4.2）。

**场景 ID 体系**：每个测试项有稳定 ID `WSTG-<类别码>-<编号>`，例如 `WSTG-ATHN-01` 表示认证类第 1 项。引用时建议带版本：`WSTG-v42-ATHN-01`；不带版本默认指最新内容。12 个类别码见父页（INFO/CONF/IDNT/ATHN/ATHZ/SESS/INPV/ERRH/CRYP/BUSL/CLNT/APIT）。
...
- **SDLC 五阶段测试模型**（5.0 强调把安全测试嵌入全生命周期）：
  - *Phase 1 开发前*：威胁建模、定安全需求。
  - *Phase 2 设计期*：评审架构与设计的安全假设。
  - *Phase 3 开发期*：代码审查、单元/组件安全测试。
  - *Phase 4 部署期*：配置核查、基础设施测试。
  - *Phase 5 运维期*：持续监控、回归测试。
- **测试技术谱系**：手动审查/评审、威胁建模、源码审计、渗透测试——强调"平衡组合"而非单靠一种。
...
- **误区 4**：ID 永远不变。版本间可能调整，引用务必带版本号（如 `WSTG-v42-…`）。
